...
BOOL CThreadPool::Init(int nThreadNums)
{
	...
	::InitializeCriticalSection(&m_csThreadInfo);      // 初始化临界区
	...
	::EnterCriticalSection(&m_csThreadInfo);   // 进入临界区
	for (int i = 0; i < nThreadNums; i++)      // 创建 nThreadNums 个线程
	{
		lpThreadInfo = new TP_THREAD_INFO;
		if (lpThreadInfo != NULL)
		{
			hThread = (HANDLE)(ULONG_PTR)_beginthreadex(NULL, 0,  
				&ThreadProc, lpThreadInfo, CREATE_SUSPENDED, (UINT*)&dwThreadID);  // 线程被创建后处于暂停状态
			if (hThread != NULL)
			{
				...
				::ResumeThread(hThread);  // 恢复一个被暂停的线程，使其继续执行
			}
			else
			{
				delete lpThreadInfo;
	...
	::LeaveCriticalSection(&m_csThreadInfo);   // 离开临界区
